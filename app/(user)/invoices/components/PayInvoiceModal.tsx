...
import { Wallet, CreditCard, Info, Plus, CheckCircle2, Loader2 } from "lucide-react"
            ...
            <DialogContent className="max-w-md p-0 overflow-hidden rounded-2xl border-none shadow-2xl">
                <div className="bg-primary p-6 text-white">
                    ...
                    <div className="mt-6 flex flex-col items-center">
                        <span className="text-sm font-medium text-blue-100 uppercase tracking-widest">Amount Due</span>
                        <div className="text-4xl font-black mt-1">
                            ${amount.toFixed(2)} <span className="text-lg font-bold opacity-80">{currency}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 space-y-6 bg-white">
                    ...
                    <div className={cn(
                        ...
                        canPayWithBalance ? "border-slate-100 hover:border-blue-200 hover:bg-blue-50/30" : "opacity-60 cursor-not-allowed border-slate-100 bg-slate-50"
                    )}>
                        <div className="flex items-start gap-4">
                            <div className={cn(
                                "w-10 h-10 rounded-lg flex items-center justify-center shadow-sm",
                                canPayWithBalance ? "bg-green-50 text-green-600" : "bg-slate-200 text-slate-400"
                            )}>
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-slate-500 mt-1">
                                    Current Balance: <span className="font-bold text-slate-900">${balance.toFixed(2)} {currency}</span>
                                </p>
                        ...
                        {canPayWithBalance && (
                            <Button className="w-full mt-4 bg-green-600 hover:bg-green-700 text-white font-bold h-10 shadow-md shadow-green-100">
                                Pay with Balance
                            </Button>
                        )}
                    </div>
                    ...
                    <div className="pt-2 text-center">
                        <p className="text-[10px] text-slate-400 leading-relaxed px-4">
                            By clicking pay, you authorize ENorth Logistics to charge your selected payment method for the full amount of <span className="font-bold">${amount.toFixed(2)}</span>.
                        </p>
                    </div>
                </div>
            </DialogContent>
